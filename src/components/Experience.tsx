    ...
    <ol className="relative border-l border-slate-700/80">
      {items.map((item, i) => (
        <li key={i} className="mb-10 ml-6">
          <span className="absolute -left-[7px] mt-1.5 h-3 w-3 rounded-full border-2 border-slate-900 bg-brand shadow-[0_0_10px_rgba(56,189,248,0.7)]" />
          <Reveal direction={direction} delay={i * 130}>
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <div className="grid gap-12 md:grid-cols-2">
          <div>
            ...
            <Timeline items={experience} direction="left" />
          </div>
          <div>
            <h3 className="mb-6 font-orbitron text-xl text-white">Formación</h3>
            <Timeline items={education} direction="right" />
          </div>
        </div>
      </div>
    </section>
